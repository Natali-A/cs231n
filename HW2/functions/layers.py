...
def fc_backward(dout, cache):
    ...
    ###########################################################################
    dx = np.dot(dout, w.T)             # dx = dout*W
    dx = dx.reshape(x.shape)           # Switch from (N,D) dimensions to original (N,d1,...,d_k)

    N = x.shape[0]
    X_reshaped = x.reshape(N, -1)
    dw = np.dot(X_reshaped.T, dout)    # dw = dout*X

    db = np.sum(dout, axis=0)          # db = dout*ones	
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
